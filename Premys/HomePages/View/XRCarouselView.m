...
@property (nonatomic, strong) XRCustomView *customView;
...
- (instancetype)initWithFrame:(CGRect)frame pageImageArr:(NSArray *)pageImageArr{
    //图片类型数组
    self.pageImageArr = pageImageArr;
...
- (void)initSubView {
    ...
    [self.pageControl addSubview:self.customView];
...
- (XRCustomView *)customView{
    if (!_customView) {
        _customView = [[XRCustomView alloc] initWithFrame:CGRectMake(0, 0, 15, 5) pageImageArr:self.pageImageArr];
    }
    return _customView;
}
...
- (void)nextPage {
    ...
    if (_changeMode == ChangeModeFade) {
        ...
        [UIView animateWithDuration:1.2 animations:^{
            ...
            [self.customView changeCurrentPageValue:_nextIndex];
        
//            DLog(@"next page  005 ----%ld", _nextIndex);
...
- (void)changeCurrentPageWithOffset:(CGFloat)offsetX {
    if (offsetX < self.width * 1.5) {
        ...
        if (index == 4) {
            [self.customView changeCurrentPageValue:4];
            self.currIndex = 4;
        }else{
            [self.customView changeCurrentPageValue:index];
        }
        
//        DLog(@"next page  001 ----%ld", index);
        
    } else if (offsetX > self.width * 2.5){
        //next Page
        _pageControl.currentPage = (self.currIndex + 1) % self.images.count;
        if ((self.currIndex + 1) % self.images.count == 4) {
            [self.customView changeCurrentPageValue:1];
            self.currIndex = -1;
        }else{
            [self.customView changeCurrentPageValue:(self.currIndex + 1) % self.images.count];
        }

//        DLog(@"next page  002 ----%ld", (self.currIndex + 1) % self.images.count);

    } else {
        //next Page
        _pageControl.currentPage = self.currIndex;
        [self.customView changeCurrentPageValue:(self.currIndex)];
//        DLog(@"next page  003 ----%ld", self.currIndex);
...
- (void)changeToNext {
    ...
    if (self.currIndex == 4) {
        [self.customView changeCurrentPageValue:0];
//        DLog(@"next page  004 ----0");

    }else{
        [self.customView changeCurrentPageValue:self.currIndex];
//        DLog(@"next page  004 ----%ld", self.currIndex);
